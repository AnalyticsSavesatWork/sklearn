...
def lars_path(X, y, Xy=None, Gram=None, max_iter=500, alpha_min=0,
              method='lar', copy_X=True, eps=np.finfo(np.float64).eps,
    ...
    eps : float, optional
        ...
        systems. By default, ``np.finfo(np.float64).eps`` is used.
...
def _lars_path_solver(X, y, Xy=None, Gram=None, n_samples=None, max_iter=500,
                      ...
                      eps=np.finfo(np.float64).eps, copy_Gram=True, verbose=0,
    ...
    tiny32 = np.finfo(np.float6432).tiny  # to avoid division by 0 warning
    equality_tolerance = np.finfo(np.float6432).eps
...
class Lars(MultiOutputMixin, RegressorMixin, LinearModel):
    ...
    eps : float, optional
        ...
        By default, ``np.finfo(np.float64).eps`` is used.
    ...
    def __init__(self, fit_intercept=True, verbose=False, normalize=True,
                 ...
                 eps=np.finfo(np.float64).eps, copy_X=True, fit_path=True):
...
def _lars_path_residues(X_train, y_train, X_test, y_test, Gram=None,
                        ...
                        eps=np.finfo(np.float64).eps, positive=False):
...
class LarsCV(Lars):
    ...
    def __init__(self, fit_intercept=True, verbose=False, max_iter=500,
                 ...
                 max_n_alphas=1000, n_jobs=None, eps=np.finfo(np.float64).eps,
                 copy_X=True):
        self.max_iter = max_iter
        self.cv = cv
